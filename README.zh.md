...
**macOS（Apple Silicon）：** 下载 `SayKnow-Kit_0.2.26_aarch64.dmg`，打开后将应用拖入 `/Applications`。v0.2.26 应用仅使用**临时签名（ad-hoc）**，没有 Developer ID 或 notarization，因此出现 Gatekeeper 警告是正常的。macOS 13 及以上版本不再接受右键绕过：先运行一次,然后前往**系统设置 → 隐私与安全性 → 仍要打开**。也可以自行清除隔离属性:
...
**Windows（x64）：** 下载 `SayKnow-Kit_0.2.26_x64-setup.exe`（NSIS）或 `SayKnow-Kit_0.2.26_x64_en-US.msi` 并运行。安装程序**未签名**，出现 SmartScreen 警告是正常的。确认官方 Release 与 `SHA256SUMS.txt` 后，再选择“更多信息 → 仍要运行”。
...
安装程序仅从官方 GitHub Release 获取，并使用 `SHA256SUMS.txt` 校验。v0.2.26 的 macOS 应用仅使用临时签名，没有 Developer ID 或 notarization；Windows 安装程序未使用 Authenticode 签名。Gatekeeper 和 SmartScreen 警告是正常现象。
