...
- 📊 **系统状态** — 显示系统 CPU、内存和系统磁盘。v0.2.5 没有经过验证的温度适配器，因此 CPU 封装温度显示为不可用
...
**macOS（Apple Silicon）：** 下载 `SayKnow Kit_0.2.5_aarch64.dmg`，打开后将应用拖入 `/Applications`。v0.2.5 应用仅使用**临时签名（ad-hoc）**，没有 Developer ID 或 notarization，因此出现 Gatekeeper 警告是正常的。在 Finder 中右键应用 → **打开** → 再次**打开**。

**Windows（x64）：** 下载 `SayKnow Kit_0.2.5_x64-setup.exe`（NSIS）或 `SayKnow Kit_0.2.5_x64_en-US.msi` 并运行。安装程序**未签名**，出现 SmartScreen 警告是正常的。确认官方 Release 与 `SHA256SUMS.txt` 后，再选择“更多信息 → 仍要运行”。
...
安装程序仅从官方 GitHub Release 获取，并使用 `SHA256SUMS.txt` 校验。v0.2.5 的 macOS 应用仅使用临时签名，没有 Developer ID 或 notarization；Windows 安装程序未使用 Authenticode 签名。Gatekeeper 和 SmartScreen 警告是正常现象。
