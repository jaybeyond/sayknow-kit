...
- 🧹 **アプリ内クリーンアップ（macOS）** — 非対話・認証なしの動作を検証済みの Mole **1.38.1** が必要です。外部の Terminal やログインシェルは使わず、ユーザー権限で一度に1つずつ実行し、管理者권限が必要な処理はスキップします。検証していない他バージョンは実行を拒否します。プレビューは削除しません（Mole 自身のレポート書き込みは行われます）。実際のクリーンアップ/最適化はユーザーのファイルと設定を変更します。タイムアウトやアプリ終了時は、アプリが起動したプロセスグループだけを終了します。
