...
- 📊 **システムステータス** — システムCPU・メモリ・システムディスクに加え、macOS（Apple Silicon）では権限昇格なしで読み取るSoCダイ温度を表示。Windowsや信頼できるセンサーを読めない場合は利用不可と表示
- 📊 **エージェント使用量（ツールタブ内）** — Claude Code / Codex / SayKnow CLI がローカルに残すセッションログを直接読み取り。5 時間の課金ブロック(残り時間・消費レート)と、CLI が記録した 5 時間・週間の実際の使用率をリセット時刻付きで表示。リセット済みのウィンドウは取り消し線で明示し、現在値としては描画しない。通信も追加ログインも不要
- 📋 **クリップボード履歴タブ** — コピー内容をバックグラウンド収集。直接書けるメモも同じ一覧に表示。本文と注釈を横断検索、ピン留め、翻訳タブへ送信、2 段階の削除。空文字・OTP 形式・PEM 鍵ブロックは保存対象外
...
- 🧹 **アプリ内クリーンアップ（macOS）** — 非対話・認証なしの動作を検証済みの Mole **1.38.1** が必要です。外部の Terminal やログインシェルは使わず、ユーザー権限で一度に1つずつ実行し、管理者権限が必要な処理はスキップします。検証していない他バージョンは実行を拒否します。プレビューは削除しません（Mole 自身のレポート書き込みは行われます）。実際のクリーンアップ/最適化はユーザーのファイルと設定を変更します。タイムアウトやアプリ終了時は、アプリが起動したプロセスグループだけを終了します。
- 🆕 **新バージョンの確認** — 1日1回 GitHub の最新リリースを確認し、歯車にドットを表示してチェックサム付きのリリースページへ案内します。裏で勝手にインストールはしません。macOS はアドホック署名、Windows は未署名のため、ダウンロードは利用者の明示的な操作のままにしています。
...
**macOS（Apple Silicon）:** `SayKnow-Kit_0.3.2_aarch64.dmg` を開き、アプリを `/Applications` へドラッグします。v0.3.2アプリは**アドホック署名のみ**で、Developer IDもnotarizationもないため、Gatekeeperの警告は想定内です。macOS 13 以降では右クリックの回避策は使えません。一度起動してから **システム設定 → プライバシーとセキュリティ → このまま開く** を選びます。または隔離属性を自分で外します:

```bash
xattr -dr com.apple.quarantine "/Applications/SayKnow Kit.app"
```

許可したのにアクセシビリティを要求され続ける場合、保存済みのエントリが更新後のアドホック署名と一致していません。次でリセットしてアプリを再起動してください:
...
**Windows（x64）:** `SayKnow-Kit_0.3.2_x64-setup.exe`（NSIS）または `SayKnow-Kit_0.3.2_x64_en-US.msi` を実行します。インストーラーは**未署名**で、SmartScreenの警告は想定内です。公式Releaseと`SHA256SUMS.txt`を確認した後に「詳細情報 → 実行」を選択してください。
...
4. アイコンクリックまたは `⌃⌥S`（macOS）/ `Ctrl+Alt+Shift+S`（Windows）で起動 → テキスト入力 → 1.5 秒後に翻訳（アイコンを右クリック → 終了）

### ショートカット

| キー | 動作 |
|---|---|
| `⌃⌥S` (macOS) | ポップオーバーの開閉（グローバル） |
| `Ctrl+Alt+Shift+S` (Windows) | ポップオーバーの開閉（グローバル） |
| `⌃⌥1`–`⌃⌥4` (macOS) / `Ctrl+Alt+Shift+1`–`4` (Windows) | 翻訳 / チャット / クリップボード / ツールで直接開く（グローバル） |
| `⌃⌥M` (macOS) / `Ctrl+Alt+Shift+M` (Windows) | どこからでも新規メモ（グローバル） |
| `⌘1`–`⌘4` (macOS) / `Ctrl+1`–`4` (Windows) | タブ切り替え |
| `⌘/` (macOS) / `Ctrl+/` (Windows) | 全ショートカット — 設定 → ショートカット |
| `⌘⏎` (macOS) | 即座に翻訳（手動モード） |
...
- **ショートカット** — 全ショートカットを使う場所ごとに一覧、他アプリが使用中のグローバルキーは表示
...
インストーラーは公式GitHub Releaseからのみ取得し、`SHA256SUMS.txt`を検証してください。v0.3.2のmacOSアプリはDeveloper ID・notarizationなしのアドホック署名で、WindowsインストーラーはAuthenticodeなしの未署名です。GatekeeperとSmartScreenの警告は想定内です。
