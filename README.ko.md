...
- 📊 **시스템 상태** — 시스템 CPU·메모리·시스템 디스크를 표시합니다. v0.2.4에는 검증된 온도 어댑터가 없어 CPU 패키지 온도는 사용할 수 없음으로 표시합니다.
...
### v0.2.4 공식 설치 프로그램 (권장)

[공식 GitHub Release](https://github.com/jaybeyond/sayknow-kit/releases)에서만 다운로드하고 `SHA256SUMS.txt`로 확인하세요.

**macOS (Apple Silicon):** `SayKnow Kit_0.2.4_aarch64.dmg`를 열어 앱을 `/Applications`로 드래그합니다. v0.2.4 앱은 **임시(ad-hoc) 서명만** 적용되며 Developer ID와 공증(notarization)이 없으므로 Gatekeeper 경고가 예상됩니다. Finder에서 우클릭 → **열기** → **열기**를 다시 선택하세요.

**Windows (x64):** `SayKnow Kit_0.2.4_x64-setup.exe`(NSIS) 또는 `SayKnow Kit_0.2.4_x64_en-US.msi`를 실행합니다. 설치 프로그램은 **서명되지 않았으며** SmartScreen 경고가 예상됩니다. 공식 Release와 `SHA256SUMS.txt` 확인 후에만 “추가 정보 → 실행”을 선택하세요.
...
설치 프로그램은 공식 GitHub Release에서만 받고 `SHA256SUMS.txt`를 확인하세요. v0.2.4 macOS 앱은 Developer ID·공증 없이 임시(ad-hoc) 서명되며, Windows 설치 프로그램은 Authenticode 없이 서명되지 않았습니다. Gatekeeper와 SmartScreen 경고가 예상됩니다.
