...
### 클립보드
- 📋 **클립보드 히스토리** — 복사한 내용을 백그라운드로 수집, 본문과 주석을 함께 검색
- 🗒️ **메모** — 복사 없이 메모지처럼 직접 작성·수정, 클립과 한 목록에 섞여 보이고 클릭하면 복사. `전체 / 클립 / 메모`로 골라 보기, 자동 삭제되지 않음
- 📝 **주석** — 왜 남겼는지 클립마다 적어둘 수 있음
- 📌 **고정** — 고정한 항목은 개수 제한과 삭제를 모두 통과
- ➡️ **번역창으로 보내기** — 항목을 바로 번역 탭으로 전달
- 🧹 **2단계 삭제** — 고정·메모 빼고 지우기 / 전체 삭제
...
- 📋 **클립보드 자동 가져오기** — macOS에서는 `⌃⌥S`, Windows에서는 `Ctrl+Alt+Shift+S`로 열 때 다른 앱에서 복사한 텍스트 자동 채움
...
- 🆕 **새 버전 확인** — 하루 한 번 GitHub 최신 릴리스를 확인해 설정 톱니에 점으로 표시하고, 체크섬이 함께 있는 릴리스 페이지로 보냅니다. 몰래 설치하지 않습니다. macOS는 ad-hoc 서명, Windows는 미서명 설치 파일이라 내려받는 것은 사용자의 선택으로 남깁니다.
...
### v0.3.3 공식 설치 프로그램 (권장)

[공식 GitHub Release](https://github.com/jaybeyond/sayknow-kit/releases)에서만 다운로드하고 `SHA256SUMS.txt`로 확인하세요.

**macOS (Apple Silicon):** `SayKnow-Kit_0.3.3_aarch64.dmg`를 열어 앱을 `/Applications`로 드래그합니다. v0.3.3 앱은 **임시(ad-hoc) 서명만** 적용되며 Developer ID와 공증(notarization)이 없으므로 Gatekeeper 경고가 예상됩니다. macOS 13 이상에서는 우클릭 우회가 통하지 않습니다. 앱을 한 번 실행한 뒤 **시스템 설정 → 개인정보 보호와 보안 → 그래도 열기**를 누르세요. 또는 격리 속성을 직접 제거해도 됩니다:
...
**Windows (x64):** `SayKnow-Kit_0.3.3_x64-setup.exe`(NSIS) 또는 `SayKnow-Kit_0.3.3_x64_en-US.msi`를 실행합니다. 설치 프로그램은 **서명되지 않았으며** SmartScreen 경고가 예상됩니다. 공식 Release와 `SHA256SUMS.txt` 확인 후에만 “추가 정보 → 실행”을 선택하세요.
...
1. 트레이 클릭 또는 macOS `⌃⌥S` / Windows `Ctrl+Alt+Shift+S` → 팝업 열림 (아이콘 우클릭 → 종료)
...
| 단축키 | 동작 |
|---|---|
| `⌃⌥S` (macOS) | 팝업 열기/닫기 (글로벌) |
| `Ctrl+Alt+Shift+S` (Windows) | 팝업 열기/닫기 (글로벌) |
| `⌃⌥1`–`⌃⌥4` (macOS) / `Ctrl+Alt+Shift+1`–`4` (Windows) | 번역 / 채팅 / 클립보드 / 도구 탭으로 바로 열기 (글로벌) |
| `⌃⌥M` (macOS) / `Ctrl+Alt+Shift+M` (Windows) | 어디서나 바로 새 메모 (글로벌) |
| `⌘1`–`⌘4` (macOS) / `Ctrl+1`–`4` (Windows) | 탭 이동 |
| `⌘/` (macOS) / `Ctrl+/` (Windows) | 전체 단축키 보기 — 설정 → 단축키 |
| `⌘⏎` (macOS) | 수동 모드에서 즉시 번역 |
...
- **단축키** — 모든 단축키를 쓰는 곳별로 정리, 다른 앱이 이미 쓰는 전역 키는 표시
- **연결** — 기본 모델, 폴백 모델, 로그아웃
...
설정 → 일반 → macOS에서는 `⌃⌥S`, Windows에서는 `Ctrl+Alt+Shift+S`로 열 때 클립보드 자동 가져오기를 켠 뒤:
1. 다른 앱에서 텍스트 선택 → macOS에서는 `⌘C`, Windows에서는 `Ctrl+C`
2. macOS에서는 `⌃⌥S`, Windows에서는 `Ctrl+Alt+Shift+S` → SayKnow Kit 열림 → 입력창에 자동 채움
3. (자동 모드면) 1.5초 후 번역됨
...
설치 프로그램은 공식 GitHub Release에서만 받고 `SHA256SUMS.txt`를 확인하세요. v0.3.3 macOS 앱은 Developer ID·공증 없이 임시(ad-hoc) 서명되며, Windows 설치 프로그램은 Authenticode 없이 서명되지 않았습니다. Gatekeeper와 SmartScreen 경고가 예상됩니다.
