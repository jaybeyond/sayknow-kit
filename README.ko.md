...
### 도구
- 🌞 **화면 밝기** — 연결된 모든 화면을 하드웨어 수준에서 조절. 외부는 DDC/CI(HDMI/DP/USB-C), 내장은 허용되는 Mac에서 IOKit 백라이트
- 🔌 **화면 전원** — 외부 모니터를 끄고 다시 켜기(DDC 대기). 충전·오디오·USB는 계속 동작
- 🎚️ **한 번에 전부** — 슬라이더 하나로 모든 화면을 함께, 또는 각각 조절
- 🔍 **정직한 표시** — DDC로 밝기를 보고하지 않는 모니터도 조절은 되고, 값은 — 로 표시
