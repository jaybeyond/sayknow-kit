...
- 📊 **Estado del sistema** — CPU, memoria y disco del sistema; la temperatura del paquete de CPU se muestra como «no disponible» en v0.2.7 porque no hay un adaptador de temperatura verificado, y también en sistemas no compatibles.
...
3. La aplicación v0.2.7 solo lleva una firma **ad hoc**, sin Developer ID ni notarización; las advertencias de Gatekeeper son esperadas.
...
2. Windows SmartScreen puede mostrar una advertencia porque v0.2.7 no está firmado; es normal. No hay Authenticode.
...
- La aplicación macOS v0.2.7 usa firma ad hoc sin Developer ID ni notarización; los instaladores de Windows no tienen firma Authenticode. Las advertencias de Gatekeeper y SmartScreen son esperadas.
