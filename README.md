...
- 📊 **System status** — system CPU, memory, and system disk; v0.2.6 has no verified temperature adapter, so CPU package temperature is shown as unavailable
...
### v0.2.6 prebuilt installers (recommended)
...
**macOS (Apple Silicon):** Download `SayKnow Kit_0.2.6_aarch64.dmg`, open it, and drag `SayKnow Kit.app` into `/Applications`. The v0.2.6 app has an **ad-hoc signature only**—no Developer ID or notarization—so Gatekeeper warnings are expected. In Finder, right-click the app → **Open** → **Open** again.

**Windows (x64):** Download `SayKnow Kit_0.2.6_x64-setup.exe` (NSIS) or `SayKnow Kit_0.2.6_x64_en-US.msi`, run the installer, and follow the prompts. The installers are **unsigned**; Windows SmartScreen warnings are expected. Choose **More info → Run anyway** only after checking the official Release and `SHA256SUMS.txt`.
...
Download installers only from the official GitHub Release and verify `SHA256SUMS.txt`. The v0.2.6 macOS app is ad-hoc signed without Developer ID or notarization; Windows installers are unsigned without Authenticode. Gatekeeper and SmartScreen warnings are expected.
